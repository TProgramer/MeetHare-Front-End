...
const WebSocketPage = () => {
    ...
    const fetchStationInfo = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_serverURL}/map/getStationInfo?fixStation=${encodedFixStation}`,
        );
        ...
        setPoints((prevPoints) => [
          ...prevPoints,
          {
            ...
            latlng: new window.kakao.maps.LatLng(
              newLocation.data.latitude,
              newLocation.data.longitude,
            ),
          },
        ]);
        ...
        window.kakao.maps.load(() => {
          ...
          if (container) {
            navigator.geolocation.getCurrentPosition((point) => {
              ...
              var centerMarkerPosition = new window.kakao.maps.LatLng(
                targetStationLat,
                targetStationLong,
              );
              ...
              for (var i = 0; i < points.length; i++) {
                ...
                var customOverlay = new window.kakao.maps.CustomOverlay({
                  position: points[i].latlng,
                  content:
                    '<div style="point: absolute; text-align: center; white-space: nowrap; top: 5px;">' +
  ...
  const handleClick = () => {
    navigator.geolocation.getCurrentPosition(
      (point) => {
        ...
        setPoints((prevPoints) => [...prevPoints, newLocation]);
      },
      (error) => {
  ...
  return (
    <main className="flex w-full flex-col items-center justify-center pt-4">
      <div
        className="h-[50vh] w-full sm:h-[60vh] md:h-[70vh] lg:h-[80vh]"
