  ...
  const connectAndSubscribe = () => {
    ...
    stompClient.current.onConnect = (frame) => {
      ...
      stompClient.current?.subscribe(`/sub/channel/${channelId}`, (message) => {
        ...
        const existingPositionIndex = positionsArray.findIndex(pos => pos.sender === newLocation.sender);

        if (existingPositionIndex !== -1) {
        // 중복된 sender가 있을 경우, 가장 최신의 위치 정보로 갱신
        positionsArray[existingPositionIndex] = newLocation;

        // 지도에 있는 해당 마커를 업데이트
        setPoints((prevPoints) => {
          const updatedPoints = prevPoints.map(point => {
            if (point.title === newLocation.sender) {
              return {
                title: newLocation.sender,
                latlng: new window.kakao.maps.LatLng(newLocation.data.latitude, newLocation.data.longitude),
              };
            }
            return point;
          });
        ...
        // 중복된 sender가 없을 경우, 새로운 위치 정보를 추가
        positionsArray.push(newLocation);
              ...
              for (var i = 0; i < points.length; i++) {
                ...
                var customOverlay = new window.kakao.maps.CustomOverlay({
                  ...
                  content: '<div style="point: absolute; text-align: center; white-space: nowrap; top: 5px;">' +
                    '<div style="display: inline-block; background-color: white; padding: 5px; border-radius: 5px; box-shadow: 1px 1px 1px rgba(0, 0, 0, 0.1); font-size:12px;">' +
                    points[i].title + 
                    '</div>' +
                    '</div>',
                });
